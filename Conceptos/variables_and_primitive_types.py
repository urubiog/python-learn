...
print(
    "Hello" * 2
)  # out: "HelloHello" -> What's inside the round brackets will be evaluated and then interpretated as the argument for print()
...
a = int(
    6.6
)  # 6 -> Integers doesn't accept a decimal part, so the initial value is truncated

print(
    type(a)
)  # out: <class 'int'> -> type() function will tell us about the type of a variable

b = float(5.21214214812)
c = str(
    "Hello world"
)  # Be careful, not the same as string(Hello world), that would be an error
...
input(
    "What's your name? "
)  # In this case, the code will be executed but not stored in any variable

b = input(
    "How old are you? "
)  # input() returns the value as a string, so we have to convert it to int
b = int(b)

print(
    f"Wow! You're {b} y/o."
)  # This is a f-string, you can insert code that will be interpreted to give its value
